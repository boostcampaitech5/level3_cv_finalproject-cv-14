...
const uploadKeyImage = multer({
  storage: multerS3({
    ...
    key: (req, file, callback) => {
      ...
      const userFolder = req.user.email;
      const uniqueFileName = `${userFolder}/key/${Date.now()}_${uuid4()}${extension}`;
      callback(null, uniqueFileName);
    },
    acl: "public-read-write",
  }),
...
app.post("/register", async (req, res) => {
  ...
  if (password !== confirm_password) {
    ...
    return res.render("register.ejs", { error: "비밀번호 확인이 일치하지 않습니다." });
  }
  ...
  try {
    ...
    if (existingUser) {
      ...
      return res.render("register.ejs", { error: "이미 사용 중인 이메일입니다." });
    }
...
app.post(
  ...
  passport.authenticate("local", {
    failureRedirect: "/", // 인증 실패 시 다시 /login으로 리디렉션
...
app.get("/find", (req, res) => {
  if (req.isAuthenticated()) {
    ...
    res.render("find.ejs", { user: currentUser });
  } else {
    // 로그인되어 있지 않은 상태라면 알림창을 띄우고 로그인 페이지로 리디렉션
    res.send('<script>alert("로그인이 필요합니다."); window.location.href = "/";</script>');
  }
...
app.post("/find", uploadKeyImage.single("file_object"), async (req, res) => {
  try {
    // 업로드된 이미지 파일의 경로
    ...
    const userImagePaths = user.imagePaths;
    ...
    // Flask API의 URL
    const FLASK_API_URL = "http://101.101.219.62:30006/search"; // 적절한 Flask API의 URL로 변경

    axios
      .post(FLASK_API_URL, data)
      .then(function (response) {
        console.log("Flask 애플리케이션으로부터 받은 응답:", response.data);
        res.status(200).json(response.data);
      })
      .catch(function (error) {
        console.error("Error while communicating with Flask server.:", error.message);
        res.status(500).json({ error: "Failed to save user." });
      });
  } catch (error) {
...
//     const apiUrl = "http://101.101.219.62:30006/find";
