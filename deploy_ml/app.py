...
# MongoDB 연결 설정
client = MongoClient("")  # 여기에 MongoDB 연결 정보를 입력해주세요.
...
def search():
    try:
        ...
        print(f"userImagePaths: {userImagePaths}")
        
        
        # MongoDB에서 사용자 정보 조회
        user = collection.find_one({'email': user_email})
        if user:
            # 이미 사용자가 존재하는 경우, 해당 사용자의 imagePaths 정보 반환
            image_paths = user.get('imagePaths', [])
            album_dict = {image_info['imagePath']: image_info['embeddingVector'] for image_info in image_paths}
            # print(album_dict)
            ...
            if img is None:
                print(f"Cannot load image from S3: {file_url}")

            with torch.no_grad():
                feature = single_detection_model(img)
                if feature is None:
                    print(f"Cannot detect face in the image: {file_url}")

            # 임베딩 벡터 계산 및 저장
            embedding = get_embedding(facenet_model, feature=feature)
            key_dict[file_url] = embedding.tolist()
            print(key_dict)

            final_classification = get_result(file_url, key_dict, threshold, album_dict)
            return jsonify({'result': list(final_classification)})
        else:
